...
function App() {
  ...
  // Derive health URL from API_URL or fallback to /health
  const HEALTH_URL = API_URL.includes('/api') 
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-100 font-sans">
      ...
      {isWakingUp && (
        <div className="w-full bg-amber-50 border-b border-amber-200 text-amber-800 px-4 py-2.5 text-center text-xs sm:text-sm font-medium flex items-center justify-center gap-2 fixed top-0 left-0 z-50 shadow-sm animate-pulse">
          <span>⚡</span>
          <span>
            <strong>Server is waking up:</strong> Initial load may take up to 45 seconds while Render spins up...
          </span>
        </div>
      )}
